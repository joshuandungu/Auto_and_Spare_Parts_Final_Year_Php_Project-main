...
class Customers_view
{
	...
	public function getCustomersOrderView()
	{
		...
		$query = $this->con->query(
			"SELECT o.order_id, o.product_id, o.qty, o.payment_status,o.order_status, p.product_title, p.product_image FROM orders o JOIN products p ON o.product_id = p.product_id where o.user_id='$id_us'"
		);
		$ar = [];
		if (@$query->num_rows > 0) {
